...
def registrarUsuario():
    ...
    rep_re=controlador.regisUsuario(nombre,correo, password2,codigo2)
    
    if rep_re=="1":
        asunto="codigo de activacion"
        mensaje ="codigo de activacion"+codigo2;
        resp_correo=envioemail.enviar(correo,asunto,mensajes)
        
        if (resp_correo=="1"):
            mensajes="Usuario registrado satifactriamente..."
        else:
            mensajes="Usuario registrado con exito Emaill no enviado,servicio no disponible utiliz el siguiente codigo d activacion="+codigo2
    else:
        mensajes="ERROR, no es posible realizar el registro, el Usuari y/o correo ya existe"
